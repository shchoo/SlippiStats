...
const statsController = (db) => {
    ...
    const getStats = async (req, res) => {
        ...
        if (req.params.code) {
            ...
            if (!code.match(/^[a-zA-Z\d]{1,6}#\d{1,6}$/) || code.length > 8) {
        ...
        if (req.query.opponentCode) {
            ...
            opponentCode = req.query.opponentCode.replace('-', '#').toLocaleUpperCase();
            if (!opponentCode.match(/^[a-zA-Z\d]{1,6}#\d{1,6}$/) || opponentCode.length > 8) {
    ...
    const getStatsGlobal = async (req, res) => {
        ...
        if (!req.query.characters) {
            return res.status(400).send({ message: 'Character is required.' });
        }

        // Get characters
        var characters = [];
        let err = false
        let character = req.query.characters;
        let charIndex = characterList.indexOf(character.toLocaleLowerCase());
        if (charIndex == -1) {
            err = true;
        }
        if (charIndex == 18 || charIndex == 19) {  // Zelda / Sheik
            characters.push(18);
            characters.push(19);
        }
        else {
        ...
        if (req.query.opponentCharacters) {
            opponentCharacters = [];
            let character = req.query.opponentCharacters;
            let err = false
            let charIndex = characterList.indexOf(character.toLocaleLowerCase());
            if (charIndex == -1) {
                err = true;
            }
            else if (characters.indexOf(charIndex) !== -1) {  // Check for ditto
                return res.status(400).send({ message: 'Mirror matches (dittos) are not allowed in global searches.' });
            }
            if (charIndex == 18 || charIndex == 19) {
                opponentCharacters.push(18);
                opponentCharacters.push(19);
            }
            else {
                opponentCharacters.push(charIndex);
            }
        ...
        games.forEach(game => {
            ...
            if (characters.indexOf(game.p1Character) != -1) {
                playerIndex = 0;
                opponentIndex = 1;
            }
            else if (characters.indexOf(game.p2Character) != -1) {
                playerIndex = 1;
                opponentIndex = 0;
            }
